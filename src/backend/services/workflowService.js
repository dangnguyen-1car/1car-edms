...
// const config = require('../config') // Đã loại bỏ do không được sử dụng
...
class WorkflowService {
  ...
  static async getWorkflowHistory (documentId, context = {}) {
    ...
    try {
      ...
      await AuditService.log({
        action: 'WORKFLOW_HISTORY_VIEWED', // Hoặc 'WORKFLOW_HISTORY_VIEWED'
        userId,
        resourceType: 'workflow_history',
        resourceId: documentId,
  ...
  static async getWorkflowStatistics (filters = {}, context = {}) {
    ...
    try {
      ...
      await AuditService.log({
        action: 'WORKFLOW_STATS_VIEWED', // Hoặc 'WORKFLOW_STATS_VIEWED'
        userId,
        resourceType: 'workflow_statistics',
  ...
  static async getPendingApprovals (userId, context = {}) { //
    ...
    try {
      ...
      await AuditService.log({
        action: 'PENDING_APPROVALS_VIEWED', // Hoặc 'PENDING_APPROVALS_VIEWED'
        userId,
  ...
  static async getWorkflowDashboard (userId, context = {}) { //
    ...
    try {
      ...
      await AuditService.log({
        action: 'WORKFLOW_DASHBOARD_VIEWED', // Hoặc 'WORKFLOW_DASHBOARD_VIEWED'
        userId,
        resourceType: 'workflow_dashboard',
