...
const { logError, createAuditLog } = require('../utils/logger')
...
class UserService {
  ...
  static async updateUser (id, updateData, updatedBy = null) {
    try {
      ...
      await dbManager.run(
        `UPDATE users SET ${setClauses}, updated_at = datetime('now', 'localtime') WHERE id = ?`,
        params
      )
      ...
      await createAuditLog({
        ...
        details: {
          ...
          // newValues: fieldsToUpdate // Giữ nguyên dòng này nếu muốn log
        }
      })
    ...
    try {
      await dbManager.run(`
        ...
        SET failed_login_attempts = failed_login_attempts + 1,
            locked_until = CASE
              WHEN failed_login_attempts >= 4 THEN datetime('now', '+30 minutes', 'localtime')
              ELSE locked_until
            END
        WHERE id = ?
      `, [userId])
    ...
    try {
      const stats = await dbManager.all(`
        SELECT
          ...
          COUNT(CASE WHEN last_login >= datetime('now', '-30 days', 'localtime') THEN 1 END) as recent_active_users
        FROM users
        GROUP BY department
    ...
    try {
      const stats = await dbManager.get(`
        SELECT
          ...
          COUNT(CASE WHEN role = 'admin' THEN 1 END) as admin_users,
          COUNT(CASE WHEN role = 'user' THEN 1 END) as regular_users,
          COUNT(CASE WHEN last_login >= datetime('now', '-7 days', 'localtime') THEN 1 END) as recent_logins,
          COUNT(CASE WHEN created_at >= datetime('now', '-30 days', 'localtime') THEN 1 END) as new_users_month,
          COUNT(CASE WHEN failed_login_attempts >= 3 THEN 1 END) as locked_accounts,
          COUNT(CASE WHEN locked_until > datetime('now', 'localtime') THEN 1 END) as currently_locked
        FROM users
      `)
  ...
  static async getLockedUsers () {
    try {
      const lockedUsers = await dbManager.all(`
        ...
        FROM users
        WHERE locked_until > datetime('now', 'localtime') OR failed_login_attempts >= 5
