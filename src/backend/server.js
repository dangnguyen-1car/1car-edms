/**
 * =================================================================
 * EDMS 1CAR - Main Server (Fixed Middleware Function Error and Added System Info to Health Check)
...
const os = require('os'); // Thêm module 'os' để lấy thông tin hệ điều hành
...
// FIXED: Create request logger middleware function properly
...
// FIXED: Use the properly defined request logger middleware
...
app.get('/health', async (req, res) => { //
  try {
    ...
    // *** THÊM THÔNG TIN SYSTEM VÀO ĐÂY ***
    const systemInfo = {
      platform: os.platform(),
      arch: os.arch(),
      nodeVersion: process.version,
      // Bạn có thể thêm các thông tin khác như tên ứng dụng, môi trường,...
      appName: "1CAR-EDMS", 
    ...
    res.status(200).json({ //
      ...
      version: '1.0.0', //
      system: systemInfo, // *** TRƯỜNG SYSTEM ĐÃ ĐƯỢC THÊM ***
      database: { //
...
// FIXED: Create error handler middleware function properly
function errorHandler(err, req, res, next) { //
  try {
    ...
    appLogger.error('Error handler failed', { error: handlerError.message }); //
    res.status(500).json({ //
      success: false, //
...
// FIXED: Use the properly defined error handler middleware
